## Writeback Email Address to Aeries SIS for Students
...
$Query = "SELECT STU.SC, STU.SN, STU.ID, STU.SEM FROM dbo.STU";
...
foreach($i in $aeriesData)
{
    try{
		...
		if($ADMail -ne '' -and $ADMail -ne $Null)
		{
			if($i.SEM -ne $ADMail)
			{
				$query = "UPDATE dbo.STU SET SEM = '$($ADMail)'  WHERE ID = '$($i.ID)'"
